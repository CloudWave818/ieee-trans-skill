...
Source-level reading is optional during ordinary planning. The 29 user-preferred original PDFs are distributed in `../papers/preferred_29` (relative to this config directory), with exact filenames and SHA-256 identifiers in `../references/preferred_29/papers.json`. Open the specified PDF page when source-level verification is needed. The older local corpus can also be found in the sibling `IEEE_TRANS_CORPUS` folder when available.
