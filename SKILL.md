...
### Framework-specific style default

For method, network, or system architecture figures, including non-UAV domains, read `references/FRAMEWORK_STYLE_PLAYBOOK.md` and use `PASTEL_LAYERED_FRAMEWORK` unless the current task specifies another style or an existing figure is explicitly locked. This is a figure-type-specific user preference, not an IEEE-wide formatting rule. Scientific fidelity and actual venue constraints remain mandatory. For the applicable architecture figure only, this default takes precedence over the preferred-29 general palette/layout; other figure families retain their own routing.

Use pale functional groups with thin dashed borders, bold serif headings, compact solid-border nested modules, fine dark directed connectors, and small manuscript-specific representation/operation insets. Show the actual changed operation rather than a chain of generic large boxes. Do not inherit the reference's Transformer, GRU, embedding, actor-critic, sampling, loss, or robot/human story unless supported by the manuscript. Repeated stacks, recurrence, gradients, and training/deployment interfaces require explicit scientific support.

In the applicable entry of `PAPER_FIGURE_DESCRIPTION.md` or its standalone brief, add `style_id`, borrowed visual grammar, do-not-copy boundary, final-size design tokens, and a source-grounded `FROZEN_STORYBOARD` covering nodes, ports, edges, labels, and allowed insets. Apply `RENDER_LOCK` before visual refinement. Supply the populated prompt and applicable style parameters from the playbook with any standalone web-chat/designer handoff; a repository path alone is not a portable specification.

Run the playbook's framework audit alongside `audits/FIGURE_TABLE_AUDIT.md`. Distinguish specification completion from actual rendering and final-size inspection. This default does not restyle motivation illustrations, data plots, trajectory results, or photographs, and does not expand the Skill's writing-venue scope.
