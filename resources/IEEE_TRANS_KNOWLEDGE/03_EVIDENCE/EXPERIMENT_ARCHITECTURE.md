# Experiment Architecture

## Evidence basis

Evidence architecture is claim-conditioned. Statistics below use the 100 A-level papers unless a narrower eligible population is stated. Boolean negatives mean *not detected by the conservative extractor*, not confirmed absence.

## Observed patterns

| Pattern | Evidence | Ratio |
|---|---:|---:|
| simulation | 95/100 | 95.0% |
| physical validation | 48/100 | 48.0% |
| ablation | 19/100 | 19.0% |
| robustness | 17/100 | 17.0% |
| generalization | 54/100 | 54.0% |
| runtime | 48/100 | 48.0% |

## Journal-conditioned view

| Journal | Evidence | Ratio |
|---|---:|---:|
| TASE | 7/7 | 100.0% |
| TCST | 6/6 | 100.0% |
| TII | 6/6 | 100.0% |
| TVT | 6/6 | 100.0% |
| TAC | 5/5 | 100.0% |
| TCYB | 5/5 | 100.0% |
| TITS | 5/5 | 100.0% |
| TIV | 5/5 | 100.0% |
| TAI | 4/4 | 100.0% |
| TCNS | 4/4 | 100.0% |
| TFS | 4/4 | 100.0% |
| TIM | 4/4 | 100.0% |
| TEVC | 3/3 | 100.0% |
| TSMCS | 3/3 | 100.0% |
| TRO | 9/10 | 90.0% |
| TAES | 5/6 | 83.3% |
| TIE | 5/6 | 83.3% |
| TMECH | 5/6 | 83.3% |
| TNNLS | 4/5 | 80.0% |

## Paper-type-conditioned view

| Primary type | Evidence | Ratio |
|---|---:|---:|
| Multi-agent decision paper | 1/1 | 100.0% |
| RL / MARL method paper | 34/35 | 97.1% |
| UAV autonomy paper | 34/35 | 97.1% |
| Autonomous vehicle paper | 14/15 | 93.3% |
| Navigation / planning paper | 12/14 | 85.7% |

## Actionable use

- Layer evidence from main comparison to mechanism isolation and then claim-specific stress, scalability/runtime, or deployment tests.
- Order evidence so each layer resolves a reviewer question left by the preceding layer.

## Applicability limits

- A strong theoretical paper may replace some empirical layers with proof; paper type controls the route.

## Traceability

Paper-level values are in `../00_META/PAPER_ANATOMY.csv`; rule-to-paper links are in `../00_META/EVIDENCE_REGISTRY.csv`. Do not convert an `ESTIMATED`, `LOWER_BOUND`, or `UNKNOWN` value into an exact claim.
